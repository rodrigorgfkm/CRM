...
foreach(getExtensiones() as $ext){
	if($ext->grupos == 1 && $ext->habilitado == 1){
		...
		foreach(getAcceso($ext->id) as $acceso)
			$script.= '<label class=\"col-xs-4\"><input  type=\"checkbox\" name=\"accesos[]\" value=\"'.$acceso->id.'\"> '.$acceso->ruta.'</label>';
		$script.= '";
';
		}
	}
...
<div class="row">
  <section class="col-lg-12">
    ...
    <form method="post" enctype="multipart/form-data" name="form" id="form" class="form-horizontal" role="form">
      <div class="form-group">
          ...
          <div class="col-xs-12 col-sm-10">
              <input type="text" name="grupo" id="grupo" class="form-control validate[required, maxSize[<?=$lim_rol?>]]">
          </div>          
      </div>
      <div class="form-group">
          <label for="" class="col-xs-12 col-sm-2 control-label">
    ...
    <? }else{
        ...
        <p><a href="index.php?option=com_erp&view=gestiongrupos" class="btn btn-success"><i class="fa fa-arrow-left"></i> Volver</a></p>
		<?
		}?>
      </div>
    </div>
