      ...
      <div class="box-body">
        <form action="" method="post" name="form" id="form" class="form-horizontal" role="form">
             <div class="form-group">
                 ...
                 <div class="col-xs-12 col-sm-4">
                    <input type="text" name="fecha" class="form-control" id="fecha" value="<?=fecha(date('Y-m-d'))?>" readonly>
                 </div>
             </div>
             <div class="form-group">
                 <label for="" class="col-xs-12 col-sm-2 control-label">
            ...
            <input type="hidden" name="id_cliente" id="id_cliente" value="<?=$comprobante->id_cliente?>" readonly>
                 ...
                 <div class="form-group" style="display:<?=$comprobante->cheque_nro!=''?'block':'none'?>">
                       <label for="" class="col-xs-12 col-sm-2 control-label">
                          Cheque Nro.
                       </label>
                       <div class="col-xs-12 col-sm-4">
                           <input name="cheque_nro" type="text" class="form-control validate[required, maxSize[<?=$lim_cheq?>]]" readonly value="<?=$comprobante->cheque_nro?>">
                       </div>
                       <label for="" class="col-xs-12 col-sm-2 control-label">
                          Banco
                       </label>
                       <div class="col-xs-12 col-sm-4" >
                            <? foreach (getLBcuentas() as $banco){
                                if($comprobante->id_banco == $banco->id){?>                                    
                                    <input name="id_banco" id="id_banco" type="text" class="form-control validate[required, maxSize[<?=$lim_cheq?>]]" readonly value="<?=$banco->banco?>">
                            <? }
                            }?>
                       </div>
                 </div>
                 <div class="form-group" style="display:<?=$comprobante->factura_nro!=''?'block':'none'?>">
                       <label for="" class="col-xs-12 col-sm-2 control-label">
                          Cheque Nro.
                       </label>
                       <div class="col-xs-12 col-sm-4">
                           <input name="factura_nro" type="text" class="form-control validate[required, maxSize[<?=$lim_cheq?>]]" readonly value="<?=$comprobante->factura_nro?>">
                       </div>
                 </div>
