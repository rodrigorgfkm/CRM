...
function newCRMProspecto(){
	...
	if($origen == 'c'){
		...
		if($id_prospecto == ''){
            ...
            `id_prospecto`, `nombre`, `apellido`, `telefono`, `celular`, `correo` 
            ) VALUES';
...
function getCRMProspectosInactivos($estado){
    ...
    $query .= $negocio.' ORDER BY fecha_registro';
    //echo $query;
    $db->setQuery($query);  
	$prospectos = $db->loadObjectList();
    return $prospectos;
...
function getCRMProspecto($id = 0){
	...
	if($cant == 0){
		$query = 'SELECT p.id, p.id_segmento, p.id_usuario, p.id_origen, p.id_asignado, p.origen, p.empresa, p.fono_empresa, p.contacto_nombre, p.contacto_apellido, p.telefono, p.celular, p.direccion, p.email, e.etapa, u.name, p.estado, e.id_negocio
		FROM #__erp_crm_prospecto AS p
		LEFT JOIN #__erp_crm_etapa AS e ON p.id = e.id_prospecto
		LEFT JOIN #__users AS u ON p.id_asignado = u.id
