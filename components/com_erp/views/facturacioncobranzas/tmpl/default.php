...
<script>
    jQuery(document).on('ready',function(){
        ...
        jQuery('body').on('click', '.cofirma',function(){
            alert('hola');
            jQuery.ajax({
                url: "index.php?option=com_erp&view=facturacioncobranzas&layout=eliminar",
              ...
              <tbody>
                 <? foreach (getFACcobranzas() as $cobranza){?>
                  <tr>
                      ...
                      <td>
                          <a href="index.php?option=com_erp&view=facturacioncobranzas&layout=edita&id=<?=$cobranza->id?>" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> <span class="hidden-xs">Editar</span></a>
                          <a class="btn btn-danger btn-sm borrar" data-id="<?=$cobranza->id?>"><i class="fa fa-trash"></i> <span class="hidden-xs">Eliminar</span></a>
                      </td>
                  </tr>
                 <? }?>
              </tbody>
...
<? require_once ('components/com_erp/views/facturacioncobranzas/tmpl/borrar.php');
}else{
    vistaBloqueada();
}
?>