...
function getExtensiones(){
	$db =& JFactory::getDBO();  
	$query = 'SELECT * FROM #__erp_extensiones ORDER BY id';
	$db->setQuery($query);  
...
function getGrupoSistema(){
	$db =& JFactory::getDBO();  
	$query = 'SELECT * FROM #__erp_grupos WHERE id = "'.JRequest::getVar('id', '', 'get').'"';
	$db->setQuery($query);  
	$grupo = $db->loadObject();
	return $grupo;
	}
function newGrupo(){
	$db =& JFactory::getDBO();
	$query = 'INSERT INTO #__erp_grupos(`id_extension`, `grupo`, `descripcion`) VALUES(';
...
function getAccesos(){
	...
	//$query = 'SELECT * FROM #__erp_rel_usuario_grupo WHERE id_usuario = "784"';
	$db->setQuery($query);  
...
function newMainEmpresa(){
	...
	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_cambio`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_cambio` (
  ...
  `cambio` decimal(11,2) NOT NULL
	...
	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_comprobante`
--
CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_comprobante` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `id_tipo` int(11) NOT NULL,
  `id_usuario` int(11) NOT NULL,
  `id_extcliente` int(11) NOT NULL,
  ...
  `revertido` tinyint(1) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;';
	$db->setQuery($query);
	$db->query();

	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_comprobante_detalle`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_comprobante_detalle` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `haber` decimal(11,2) NOT NULL
	...
	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_comprobante_tipo`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_comprobante_tipo` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `fijo` tinyint(1) NOT NULL
	...
	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_cuentas`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_cuentas` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `rel` int(1) NOT NULL,
  `eb` varchar(1) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;';
	$db->setQuery($query);
	$db->query();

	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_gestion`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_gestion` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `activa` tinyint(1) NOT NULL
	...
	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_gestiondetalle`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_gestiondetalle` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `id_gestionreporte` int(11) NOT NULL,
  `id_cuenta` int(11) NOT NULL,
  `codigo` varchar(8) NOT NULL,
  `concepto` varchar(150) NOT NULL,
  `debe` decimal(11,2) NOT NULL,
  `haber` decimal(11,2) NOT NULL
	...
	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_gestionreporte`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_gestionreporte` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `id_usuario` int(11) NOT NULL,
  `id_gestion` int(11) NOT NULL,
  `fecha` date NOT NULL
	...
	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_lcv`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_lcv` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `libro` varchar(2) NOT NULL,
  ...
  `id_impuesto` int(11) NOT NULL
	...
	$query = '--
-- Estructura de tabla para la tabla `#__erp_'.$codigo.'_conta_moneda`
--

CREATE TABLE IF NOT EXISTS `#__erp_'.$codigo.'_conta_moneda` (
  `id` int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY,
  ...
  `simbolo` varchar(10) NOT NULL
	...
	$query = '--
-- Indices de la tabla `#__erp_'.$codigo.'_conta_comprobante`
--
ALTER TABLE `#__erp_'.$codigo.'_conta_comprobante`
  ADD KEY `id_tipo` (`id_tipo`,`id_usuario`),
  ADD KEY `id_extcliente` (`id_extcliente`),
  ADD KEY `id_usuario` (`id_usuario`);';
	$db->setQuery($query);
	$db->query();
  
	$query = '--
-- Indices de la tabla `#__erp_'.$codigo.'_conta_comprobante_detalle`
--
ALTER TABLE `#__erp_'.$codigo.'_conta_comprobante_detalle`
  ...
  ADD KEY `id_cuenta` (`id_cuenta`);';
	...
	$query = '--
-- Indices de la tabla `#__erp_'.$codigo.'_conta_gestiondetalle`
--
ALTER TABLE `#__erp_'.$codigo.'_conta_gestiondetalle`
  ADD KEY `id_gestionreporte` (`id_gestionreporte`),
  ADD KEY `id_cuenta` (`id_cuenta`);';
	$db->setQuery($query);
	$db->query();
  
	$query = '--
-- Indices de la tabla `#__erp_'.$codigo.'_conta_gestionreporte`
--
ALTER TABLE `#__erp_'.$codigo.'_conta_gestionreporte`
  ADD KEY `id_usuario` (`id_usuario`);';
	$db->setQuery($query);
	$db->query();
	}
...
function getUsuario($id = 0){
	if($id == 0)
		$id = JRequest::getVar('id', '', 'get');
	$db =& JFactory::getDBO();  
	$query = 'SELECT u.*, ue.cargo, ue.foto, r.group_id 
	FROM #__users AS u 
...
function getUsuarios(){
	...
	FROM #__users AS u 
	LEFT JOIN #__user_usergroup_map AS r ON u.id = r.user_id
	WHERE id_empresa = "'.$session->get('ide').'" 
	ORDER BY r.group_id, u.name';
	$db->setQuery($query);  
	$usuarios = $db->loadObjectList();
	return $usuarios;
...
function getClientes($valido){
	...
	$where = '';
	if($_POST){
		$asociado = JRequest::getVar('asociado', JRequest::getVar('filtro_asociado', '', 'post'), 'post');
		$registro = JRequest::getVar('registro', JRequest::getVar('filtro_registro', '', 'post'), 'post');
		$id_categoria = JRequest::getVar('id_categoria', JRequest::getVar('filtro_id_categoria', '', 'post'), 'post');
	...
	$query = 'SELECT c.*, cat.categoria, cat.sigla AS categoria_sigla, s.tipo AS sociedad, s.sigla AS sociedad_sigla, i.id AS id_info, i.id_categoria, i.id_estado, i.id_usuario_cobrador, i.id_usuario_mensajero, i.capital, i.matr_recsa, i.resol_recsa, i.testimonio, i.poder, i.detalle, i.estado, i.extraviado, i.empresa, i.nit, i.atache, i.nombre, i.apellido, i.direccion, i.zona, i.ciudad, i.activo
...
function getCliente($id = 0, $id_info = 0){
	...
	$query = 'SELECT c.*, s.tipo AS sociedad, s.sigla AS sociedad_sigla, i.id AS id_info, i.id_categoria, i.id_estado, i.id_usuario_cobrador, i.id_usuario_mensajero, i.id_modo_envio, i.capital, i.mat_fundempresa, i.testimonio, i.poder, i.detalle, i.estado, i.extraviado, i.empresa, i.nit, i.atache, i.nombre, i.apellido, i.direccion, i.ciudad, i.zona, i.casilla, i.fax, i.activo, i.fecha AS fecha_cambio, i.comentario_act, i.tipo_empresa, i.per_permanente, i.per_eventual
	FROM #__erp_clientes AS c
	LEFT JOIN #__erp_clientes_sociedad AS s ON c.id_tiposociedad = s.id
	LEFT JOIN #__erp_clientes_info AS i ON c.id = i.id_cliente
...
function changeClienteEstado(){
	...
	$query = 'UPDATE #__erp_clientes_info SET id_estado = "'.$id_estado.'" WHERE id = "'.$id_info.'"';
	$db->setQuery($query);  
	$db->query();
	}
...
function validaCliente(){
	...
	if($count == 0){
		...
		`nombre`, `apellido`, `poder`, `comentario_act`
		) VALUES(';
		$query.= '"'.$id_categoria.'"';
		$query.= ', "'.$id_estado.'"';
		$query.= ', "'.$id_cobrador.'"';
		...
		$suc = JRequest::getVar('sucursales', '', 'post');
		foreach(getEstados(1) as $dep){
			$query = 'INSERT INTO #__erp_clientes_sucursales(`id_info`, `id_departamento`, `cantidad`) VALUES(';
			$query.= '"'.$id_info.'"';
			$query.= ', "'.$dep->id.'"';
			$query.= ', "'.$suc[$dep->id].'"';
			$query.= ')';
...
function newCliente(){
	...
	if($count == 0){
		...
		`ciudad`, `zona`, `activo`, `fecha`, `capital`, 
		`mat_fundempresa`, `testimonio`, `casilla`, `fax`, `detalle`,
		`nombre`, `apellido`, `poder`, `comentario_act`, `tipo_empresa`, `per_permanente`, `per_eventual`
		) VALUES(';
		$query.= '"'.$id_categoria.'"';
		$query.= ', "'.$id_estado.'"';
		$query.= ', "'.$id_cobrador.'"';
...
function getClienteSucursales($id_info, $id_departamento){
	...
	$query = 'SELECT cantidad FROM #__erp_clientes_sucursales WHERE id_info = "'.$id_info.'" AND id_departamento = "'.$id_departamento.'"';
	$db->setQuery($query);  
	$cant = $db->loadResult();
...
function getProductos($id_categoria = 0, $destacado = ''){
	...
	WHERE p.id_origen = "'.getOrigen().'" '.$where.'
	GROUP BY i.id
	ORDER BY i.fijo DESC, i.category_id ASC, i.orden ASC';
	$db->setQuery($query);  
...
function getProductosCodigo(){
	...
	LEFT JOIN #__erp_producto_unidades AS u ON i.id_unidad = u.id 
	LEFT JOIN #__erp_producto_cantidad AS cant ON i.id = cant.id_producto 
	WHERE p.id_origen = "'.getOrigen().'" AND i.published = "1" AND i.codigo LIKE "%'.JRequest::getVar('codigo', '', 'post').'%" 
	GROUP BY i.id
...
function getProducto($id = 0){
	...
	LEFt JOIN #__erp_producto_precio AS p ON i.id = p.id_producto 
	LEFT JOIN #__erp_producto_categories AS c ON i.category_id = c.id 
	WHERE p.id_origen = "'.getOrigen().'" AND i.id = "'.$id.'"';
	$db->setQuery($query);  
	$plato = $db->loadObject();
	return $plato;
	}
function newProducto(){
	...
	$empresa		= getEmpresa();
	$codigo			= JRequest::getVar('codigo', '', 'post');
	$precio_base	= JRequest::getVar('precio_base', '', 'post');
	...
	$query = 'INSERT INTO #__erp_producto_items(`category_id`, `id_ctacontable`, `codigo`, `id_unidad`, `name`, `alias`, `description`'.$query_img_field.') VALUES(';
	$query.= '"'.$category_id.'"';
	...
	$query = 'INSERT INTO #__erp_producto_precio(id_producto, id_origen, price) VALUES(';
	$query.= '"'.$id.'"';
	$query.= ', "'.$id_origen.'"';
	$query.= ', "'.$precio_base.'"';
	$query.= ')';
...
function editProducto(){
	...
	$query = 'UPDATE #__erp_producto_precio SET ';
	$query.= '`price` = "'.$price.'"';
	$query.= ' WHERE id_producto = "'.$id.'" AND id_origen = "'.$id_origen.'"';
	$db->setQuery($query);  
	$db->query();
	}
...
function getCategorias(){
	...
	WHERE 1 ORDER BY parent_id, orden, name';
	$db->setQuery($query);  
...
function printCategorias($id, $tipo, $nivel, $valor=0){
	$db =& JFactory::getDBO();  
	$query = 'SELECT * FROM #__erp_producto_categories WHERE parent_id = "'.$id.'" ORDER BY orden';
	$db->setQuery($query);  
...
function getSucursales(){
	$db =& JFactory::getDBO();
	$query = 'SELECT * FROM #__erp_sucursales WHERE 1 ORDER BY departamento, nombre';
	$db->setQuery($query);  
	$sucursales = $db->loadObjectList();
	return $sucursales;
	}
...
function newSucursal(){
	...
	$query = 'INSERT INTO #__erp_sucursales(`departamento`, `nombre`, `codigo`, `direccion`, `telefono`) VALUES(';
	$query.= '"'.$departamento.'"';
	$query.= ', "'.$nombre.'"';
	$query.= ', "'.$codigo.'"';
	$query.= ', "'.$direccion.'"';
...
function getPlantilla($id){
	$db =& JFactory::getDBO();
	$query = 'SELECT plantilla FROM #__erp_plantilla WHERE id_extension = "'.$id.'" AND predeterminado = 1';
	$db->setQuery($query);  
...
// Campos adicionales
function getCampos($empresa = 0){
	$db =& JFactory::getDBO();
	$query = 'SELECT * FROM #__erp_clientes_comunicaciontipo WHERE empresa = "'.$empresa.'" ORDER BY tipo';
	$db->setQuery($query);  
	$campos = $db->loadObjectList();
	return $campos;
	}
function getCampo($id = 0){
	if($id == 0)
		$id = JRequest::getVar('id', '', 'get');
	$db =& JFactory::getDBO();
	$query = 'SELECT * FROM #__erp_clientes_comunicaciontipo WHERE id = "'.$id.'"';
	$db->setQuery($query);  
	$campo = $db->loadObject();
	return $campo;
	}
function newCampo(){
	$db =& JFactory::getDBO();
	$query.= 'INSERT INTO #__erp_clientes_comunicaciontipo(`tipo`, `obligatorio`, `fijo`) VALUES(';
	$query.= '"'.JRequest::getVar('campo', '', 'post').'"';
	$query.= ', "'.JRequest::getVar('obligatorio', '', 'post').'"';
	$query.= ', "0"';
	$query.= ')';
	$db->setQuery($query);
	$db->query();
...
function editCampo(){
	$db =& JFactory::getDBO();
	$query.= 'UPDATE #__erp_clientes_comunicaciontipo SET ';
	$query.= '`tipo` = "'.JRequest::getVar('campo', '', 'post').'"';
	$query.= ', `obligatorio` = "'.JRequest::getVar('obligatorio', '', 'post').'"';
	$query.= ' WHERE id = "'.JRequest::getVar('id', '', 'get').'"';
	$db->setQuery($query);
	$db->query();
	}
...
function getCampoValores(){
	$db =& JFactory::getDBO();
	$query = 'SELECT SUM(id) AS cantidad FROM #__erp_clientes_comunicacion WHERE id_tipo = "'.JRequest::getVar('id', '', 'get').'" GROUP BY id_tipo';
	$db->setQuery($query);  
	$cantidad = $db->loadResult();
	return $cantidad;
	}
function changeCampo(){
	$db =& JFactory::getDBO();
	$query.= 'UPDATE #__erp_clientes_comunicaciontipo SET ';
	$query.= 'obligatorio = "'.JRequest::getVar('o', '', 'get').'"';
	$query.= ' WHERE id = "'.JRequest::getVar('id', '', 'get').'"';
	$db->setQuery($query);
	$db->query();
	}
function getCampoValor($id, $id_info, $origen = ''){
	$db =& JFactory::getDBO();
	
	if($origen == '')
		$where = 'id_cliente = "'.$id_info.'"';
		else
		$where = 'id_personal = "'.$id_info.'"';
	
	$query = 'SELECT numero FROM #__erp_clientes_comunicacion WHERE id_tipo = "'.$id.'" AND '.$where;
	$db->setQuery($query);  
	$valor = $db->loadResult();
	return $valor;
	}
...
function getStatAsociados(){
	...
	$query = 'SELECT COUNT(id) FROM #__erp_clientes WHERE registro != ""';
	$db->setQuery($query);  
	$num = $db->loadResult();
	return $num;
	}
