...
if(validaAcceso('Cuentas Contables') or validaAcceso('Registro de Productos')){
	...
	$cta_main = getCNTcuentabyname('INGRESOS');
	$id_cta = getCNTcuentabyorigen($cta_main, $id_gestion);
?>
<script>
...
function cambiaGestion(){
	...
	var id_html = '<?=$id_html?>';
	location.href = 'index.php?option=com_erp&view=contaadmcuentas&layout=listaproductos&id='+id+'&id_html='+id_html+'&aux='+aux+'&tmpl=component';
	}
</script>
<style>
    ...
    <div class="box box-success">
      ...
      <div class="box-body">
        <table class="table table-striped table-bordered datatable">
            ...
            <tbody>
				<? $n = 1;
                foreach(getCNTcuentas($id_gestion, $id_cta) as $cta){
					if($cta->codigo == 0){
                ...
                <tr>
                  ...
                  <td>
                      <? if(!hasChild($cta->id)){?>
                      <a class="btn btn-success btn-xs" onClick="envia('<?=$cta_id?>', '<?=filtroCadena2($cta_nombre)?>', '<?=$cta_codigo?>', '<?=$id[1]?>', '<?=$id_auxiliar?>')">
