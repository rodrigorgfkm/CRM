...
function adicionaFila(){
    ...
    fila+= '                                <td><input name="detalle[]" autocomplete="off" type="text" class="form-control validate[required]" id="detalle_'+n+'" title="Debe introducir el detalle*" /></td>';
    ...
    fila+= '                                <td><a class="btn btn-danger" onclick="eliminaFila('+n+')"><em class="fa fa-trash icon-white"></em></a></td></tr>';
...
function buscaCliente() {
	...
	setTimeout(function(){
		...
		jQuery.post( "index.php?option=com_erp&view=crmproforma&layout=cliente&tmpl=component", {cliente:cliente}, function(data) {
...
function cargaCliente(id, nombre, fono, celular, email, tipo, pre){
	...
	if(tipo == 'p'){
		jQuery.post( "index.php?option=com_erp&view=crmproforma&layout=empresa&tmpl=component", {id:id}, function(data) {
		  var respuesta = data.split('<!-- INICIO -->');
		  var contenido = respuesta[1].split('<!-- FIN -->');
	...
	jQuery.post( "index.php?option=com_erp&view=crmproforma&layout=vehiculo&tmpl=component", {id:id}, function(data) {
		  var respuesta = data.split('<!-- INICIO -->');
		  var contenido = respuesta[1].split('<!-- FIN -->');
...
function buscaProducto(varid) {
	...
	setTimeout(function(){
		...
		jQuery.post( "index.php?option=com_erp&view=crmproforma&layout=producto&tmpl=component", {codigo:codigo, id:id[1]}, function(data) {
		  var respuesta = data.split('<!-- INICIO -->');
		  var contenido = respuesta[1].split('<!-- FIN -->');
        ...
        <? if(!$_POST){?>
            <form method="post" enctype="multipart/form-data" name="form1" id="form1">
              <table class="table table-striped table-bordered">
                <tbody>
                  <tr>
                    <td width="20%">Cliente <i class="fa fa-asterisk text-red"></i></td>
                    <td width="30%">
                        <input type="text" autocomplete="off" name="cliente" id="cliente" class="form-control valdidate[required]" onKeyUp="buscaCliente()">
                  ...
                  <tr>
                    ...
                    <td>Celular <i class="fa fa-asterisk text-red"></i></td>
                    <td><input type="text" name="celular" id="celular" class="form-control validate[required]"></td>
                  </tr>
                  <tr>
                    <td>Correo electrónico</td>
                    <td><input type="text" name="email" id="email" class="form-control"></td>
                    <td>Fecha <i class="fa fa-asterisk text-red"></i></td>
                    <td><input type="text" name="fecha" id="fecha" class="form-control validate[required]" value="<?=date('d/m/Y')?>" readonly></td>
                  </tr>
                  <tr>
                    <td>Fec. Validez <i class="fa fa-asterisk text-red"></i></td>
                    <td>
                        <input name="validez" type="text" id="validez" size="5" maxlength="5" style="width:auto" class="form-control validate[required]">
                        <select name="validez_tipo" style="width:auto" class="form-control validate[required]">
                            <option value="días calendario">Días calendario</option>
                            <option value="días hábiles">Días hábiles</option>
                        </select>
                    </td>
                    <td>Fec. Entrega <i class="fa fa-asterisk text-red"></i></td>
                    <td>
                        <input name="entrega" type="text" id="entrega" size="5" maxlength="5" style="width:auto" class="form-control validate[required]">
                        <select name="entrega_tipo" style="width:auto" class="form-control validate[required]">
                            <option value="Días hábiles">Días hábiles</option>
                            <option value="Días calendario">Días calendario</option>
                        </select>
                    </td>
              ...
              <table class="table table-striped table-bordered" id="detalle_lista">
                ...
                <tbody>
                  <tr id="tr_0">    
                    <td>
                        <input name="codigo[]" autocomplete="off" type="text" class="form-control validate[required]" id="codigo_0" title="Debe introducir un item*" onkeyup="buscaProducto(this.id)" size="5" maxlength="8" />
                    ...
                    <td><input name="detalle[]" autocomplete="off" type="text" class="form-control validate[required]" id="detalle_0" title="Debe introducir el detalle*" /></td>
                    <td><input name="precio[]" type="text" class="form-control validate[required]" id="precio_0" title="Debe introducir el precio del producto*" onkeyup="calcula(this.id)" value="0" maxlength="8" /></td>
            ...
            <? }else{
              ...
              <table class="table table-striped table-bordered">
                <tbody>
                  ...
                  <tr>
                    <td>Fec. Validez</td>
                    <td><?=JRequest::getVar('validez', '', 'post')?></td>
                    <td>Fec. Entrega</td>
                    <td><?=JRequest::getVar('entrega', '', 'post')?></td>
                  </tr>
                  <? if($ext['veh']->habilitado == 1){?>
                  <tr>
              ...
              <table class="table table-striped table-bordered" id="detalle_lista">
                ...
                <tfoot>
                  ...
                  <tr>
                    <td colspan="6" style="text-align:center">
                      <a class="btn btn-info col-xs-12" rel="shadowbox;width=950;" id="imprime" style="font-size:14px" href="index.php?option=com_erp&view=crmproforma&layout=imprime&id=<?=$id?>&tmpl=component">Imprimir</a>
                    </td>
                  </tr>
                </tfoot>
              </table>
