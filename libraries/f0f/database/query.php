 ...
 * @property-read   F0FDatabaseQueryElement  $type
 * @property-read   F0FDatabaseQueryElement  $select
 * @property-read   F0FDatabaseQueryElement  $group
 * @property-read   F0FDatabaseQueryElement  $having
 */
abstract class F0FDatabaseQuery
{
	/**
	 * @var    F0FDatabaseDriver  The database driver.
	 * @since  11.1
	 */
	protected $db = null;
	...
	/**
	 * @var    F0FDatabaseQueryElement  The query element for a generic query (type = null).
	 * @since  11.1
	 */
	protected $element = null;

	/**
	 * @var    F0FDatabaseQueryElement  The select element.
	 * @since  11.1
	 */
	protected $select = null;

	/**
	 * @var    F0FDatabaseQueryElement  The delete element.
	 * @since  11.1
	 */
	protected $delete = null;

	/**
	 * @var    F0FDatabaseQueryElement  The update element.
	 * @since  11.1
	 */
	protected $update = null;

	/**
	 * @var    F0FDatabaseQueryElement  The insert element.
	 * @since  11.1
	 */
	protected $insert = null;

	/**
	 * @var    F0FDatabaseQueryElement  The from element.
	 * @since  11.1
	 */
	protected $from = null;

	/**
	 * @var    F0FDatabaseQueryElement  The join element.
	 * @since  11.1
	 */
	protected $join = null;

	/**
	 * @var    F0FDatabaseQueryElement  The set element.
	 * @since  11.1
	 */
	protected $set = null;

	/**
	 * @var    F0FDatabaseQueryElement  The where element.
	 * @since  11.1
	 */
	protected $where = null;

	/**
	 * @var    F0FDatabaseQueryElement  The group by element.
	 * @since  11.1
	 */
	protected $group = null;

	/**
	 * @var    F0FDatabaseQueryElement  The having element.
	 * @since  11.1
	 */
	protected $having = null;

	/**
	 * @var    F0FDatabaseQueryElement  The column list for an INSERT statement.
	 * @since  11.1
	 */
	protected $columns = null;

	/**
	 * @var    F0FDatabaseQueryElement  The values list for an INSERT statement.
	 * @since  11.1
	 */
	protected $values = null;

	/**
	 * @var    F0FDatabaseQueryElement  The order element.
	 * @since  11.1
	 */
	...
	/**
	 * @var    F0FDatabaseQueryElement  The call element.
	 * @since  12.1
	 */
	protected $call = null;

	/**
	 * @var    F0FDatabaseQueryElement  The exec element.
	 * @since  12.1
	 */
	protected $exec = null;

	/**
	 * @var    F0FDatabaseQueryElement  The union element.
	 * @since  12.1
	 */
	protected $union = null;

	/**
	 * @var    F0FDatabaseQueryElement  The unionAll element.
	 * @since  13.1
	 */
	 ...
	 * Class constructor.
	 *
	 * @param   F0FDatabaseDriver  $db  The database driver.
	 *
	 * @since   11.1
	 */
	public function __construct(F0FDatabaseDriver $db = null)
	{
		$this->db = $db;
	}
	...
	public function call($columns)
	{
		...
		{
			$this->call = new F0FDatabaseQueryElement('CALL', $columns);
		}
		else
		{
	...
	public function columns($columns)
	{
		...
		{
			$this->columns = new F0FDatabaseQueryElement('()', $columns);
		}
		else
		{
	...
	{
		if (!($this->db instanceof F0FDatabaseDriver))
		{
	...
	{
		return '<pre class="F0FDatabasequery">' . str_replace('#__', $this->db->getPrefix(), $this) . '</pre>';
	}
	...
	{
		$this->type = 'delete';
		$this->delete = new F0FDatabaseQueryElement('DELETE', null);
	 ...
	 *
	 * Note that 'e' is an alias for this method as it is in F0FDatabaseDriver.
	 *
	...
	public function exec($columns)
	{
		...
		{
			$this->exec = new F0FDatabaseQueryElement('EXEC', $columns);
		}
		else
		{
	 ...
	 *                                  This can be a F0FDatabaseQuery object (or a child of it) when used
	 *                                  as a subquery in FROM clause along with a value for $subQueryAlias.
	 * @param   string  $subQueryAlias  Alias used when $tables is a F0FDatabaseQuery.
	 *
	 * @return  F0FDatabaseQuery  Returns this object to allow chaining.
	 *
	...
	public function from($tables, $subQueryAlias = null)
	{
		if (is_null($this->from))
		{
			...
			$this->from = new F0FDatabaseQueryElement('FROM', $tables);
		}
		else
		{
	...
	public function group($columns)
	{
		...
		{
			$this->group = new F0FDatabaseQueryElement('GROUP BY', $columns);
		}
		else
		{
	...
	public function having($conditions, $glue = 'AND')
	{
		if (is_null($this->having))
		{
			...
			$this->having = new F0FDatabaseQueryElement('HAVING', $conditions, " $glue ");
		}
		else
		{
	...
	{
		$this->type = 'insert';
		$this->insert = new F0FDatabaseQueryElement('INSERT INTO', $table);
	...
	public function join($type, $conditions)
	{
		...
		$this->join[] = new F0FDatabaseQueryElement(strtoupper($type) . ' JOIN', $conditions);
	...
	public function order($columns)
	{
		...
		{
			$this->order = new F0FDatabaseQueryElement('ORDER BY', $columns);
		}
		else
		{
	 ...
	 *
	 * Note that 'q' is an alias for this method as it is in F0FDatabaseDriver.
	 *
	 * Usage:
	 ...
	 *
	 * Note that 'qn' is an alias for this method as it is in F0FDatabaseDriver.
	 *
	 * Usage:
	 * $query->quoteName('#__a');
	 * $query->qn('#__a');
	...
	public function select($columns)
	{
		...
		{
			$this->select = new F0FDatabaseQueryElement('SELECT', $columns);
		}
		else
		{
	...
	public function set($conditions, $glue = ',')
	{
		...
		{
			$glue = strtoupper($glue);
			$this->set = new F0FDatabaseQueryElement('SET', $conditions, "\n\t$glue ");
		}
		else
		{
	...
	{
		$this->type = 'update';
		$this->update = new F0FDatabaseQueryElement('UPDATE', $table);
	...
	public function values($values)
	{
		...
		{
			$this->values = new F0FDatabaseQueryElement('()', $values, '),(');
		}
		else
		{
	...
	public function where($conditions, $glue = 'AND')
	{
		...
		{
			$glue = strtoupper($glue);
			$this->where = new F0FDatabaseQueryElement('WHERE', $conditions, " $glue ");
		}
		else
		{
	 ...
	 *
	 * @param   mixed    $query     The F0FDatabaseQuery object or string to union.
	 * @param   boolean  $distinct  True to only return distinct rows from the union.
	 * @param   string   $glue      The glue by which to join the conditions.
	 *
	 * @return  mixed    The F0FDatabaseQuery object on success or boolean false on failure.
	 *
	...
	public function union($query, $distinct = false, $glue = '')
	{
		...
		// Get the F0FDatabaseQueryElement if it does not exist
		if (is_null($this->union))
		{
			$this->union = new F0FDatabaseQueryElement($name, $query, "$glue");
		}
	...
	public function unionAll($query, $distinct = false, $glue = '')
	{
		...
		{
			$this->unionAll = new F0FDatabaseQueryElement($name, $query, "$glue");
		}
