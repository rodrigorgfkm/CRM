/*
 ...
 - [Oct 2021] Prefix "akeeba-modal-" instead of generic "modal-" to avoid conflicts with 3PD software e.g. Bootstrap.
 - [Oct 2021] Remove support for AJAX content. We use our own AJAX handlers which work around 3PD plugins corrupting content.
 ? [Oct 2021] Added method show() which just calls open() for backwards compatibility reasons.
 ? [Oct 2021] Added parameter "inherit" where you can give a query selector for an element whose content will be inherited by the modal.
 ? [Oct 2021] open() returns the Modal object so that we can interact with it through external code.
 ? [Nov 2021] Added parameter "iframe" where you can give a URL to load inside an IFRAME.
*/
